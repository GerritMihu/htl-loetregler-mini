...
#define PIN_AIN_UBatt       26
#define PIN_AIN_TempSensor  27
#define PIN_AIN_Strom       28
...
// Funktion, die die aktuelle Zeit als char-Array ausgibt
void getCurrentTime(char* timeString) {
    uint64_t currentTime = time_us_64();
    uint64_t milliseconds = currentTime / 1000;
    ...
    sprintf(timeString, "%02llu:%02llu:%02llu.%03llu", hours, minutes, seconds, milliseconds);
}
...
int init_microcontroller_gpios() {
    adc_gpio_init(PIN_AIN_UBatt);
    adc_gpio_init(PIN_AIN_TempSensor);
    adc_gpio_init(PIN_AIN_Strom);
...
int selbsthaltung() {
  if (millis() < 10000 || spannungBatterie() > SPANNUNG_LEER) { // für 40V-Akku: 30V, für 20V-Akku: 15V
    digitalWrite(PIN_Selbsthaltung, 1);
  } else {
...
int main(){
    stdio_init_all();
    display_init();
    init_microcontroller_gpios();

    // The Main Loop does what ever the Main Loop does..
    for (;;)
    {
        mainScreen();
        regler();
